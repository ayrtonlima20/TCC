...
define(function () {
    ...
    function Usuario(name) {
        ...
        if (!(this instanceof Usuario)) {
            throw new TypeError("Usuario constructor cannot be called as a function.");
        }
     ...
     */
    Usuario.create = function (name, age) {
        var result = new Usuario(name);
        result.setAge(age);
     ...
     */
    function formatNameAndAge(usuario) {
        // Note that `this` does not refer to the Person object from inside this method.
        if (usuario._age === -1) {
            return "We don't know how old " + usuario.name + " is!";
        }

        return (usuario.name + ", is " + usuario._age + " years old and "
            + ((usuario.canRetire()) ? "can" : "can't") + " retire");
    };
    ...
    Usuario.prototype = {
         ...
         */
        canRetire: function() {
            return this._age >= Usuario.RETIREMENT_AGE;
        },
